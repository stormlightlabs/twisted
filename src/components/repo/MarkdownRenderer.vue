...
// Languages commonly found in README code blocks — load upfront so aliases (e.g. "sh") resolve.
const PRELOAD_LANGS = [
  "bash", "javascript", "typescript", "python", "json", "yaml",
  "html", "css", "rust", "go", "sql", "markdown", "tsx", "jsx",
] as const;
...
async function resolveImageSources(html: string, repoContext?: RepoAssetContext): Promise<{ html: string; objectUrls: string[] }> {
...
.markdown-body :deep(h1) { font-size: 2em; border-bottom: 1px solid var(--t-border); padding-bottom: 0.3em; }
.markdown-body :deep(h2) { font-size: 1.5em; border-bottom: 1px solid var(--t-border); padding-bottom: 0.3em; }
